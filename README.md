...
Set `QWEN38_PROFILE=1` to print target prefill/decode timings and an MTP summary containing draft/accepted counts, acceptance ratio, stage timings, and generated-token throughput. `QWEN38_BATCH_TIMING=1` adds encoding, submission/wait, GPU, and host-readback timings for the normal verification graph; `QWEN38_BATCH_PROFILE=1` remains a slower diagnostic mode and additionally reports separate target/adapter GPU timings for the fused MTP round. The MTP path is intentionally limited to one adapter layer and `block_size >= 2`. Although the published adapter uses `block_size=3` and can propose two tokens, the native default verifies one draft token at a time: M4 Pro's Q4 batch-2 path has higher measured output throughput and needs less transient scratch/state snapshot memory. Set `QWEN38_MTP_MAX_DRAFT_TOKENS=2` to opt into the full two-draft adapter block for device-specific experiments.

For the published two-token MTP adapter, batch-2 Q4 verification defaults to a 32-lane two-output-row tile. It reuses each activation panel across adjacent target rows, including residual projections, and handles unaligned safetensor offsets without copying or repacking model shards. A one-draft DeltaNet transaction writes its intermediate recurrent state directly into the rollback snapshot, avoiding a full state copy for every linear layer. On the tested M4 Pro this reduced target batch-2 GPU time by about 10% and raised end-to-end MTP decode from roughly 16.3 to 17.1 tokens/s at the same acceptance rate. Set `QWEN38_DISABLE_BATCH2_ROW_TILE=1` to restore the previous batch-vector path. `QWEN38_PAIR_TRACE=1` prints the paired-projection dispatch selected for each verification projection. For additional controlled A/B measurements, set any of `QWEN38_DISABLE_BATCH2_WEIGHT_VECTOR=1`, `QWEN38_DISABLE_BATCH3_WEIGHT_VECTOR=1`, `QWEN38_DISABLE_FUSED_PAIR=1`, `QWEN38_DISABLE_BATCH_SIMDGROUP=1`, or `QWEN38_DISABLE_SHORT_BATCH=1`; these switches progressively fall back to less specialized Q4 kernels.
